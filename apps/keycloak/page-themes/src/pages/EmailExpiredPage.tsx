...
export const EmailExpiredPage: React.FC = () => {
  const searchParams = new URLSearchParams(window.location.search);
  const email = searchParams.get('email') || 'usuario@openhive.io';
  const resent = searchParams.get('resent') === 'true';

  const handleRequestNewLink = () => {
    window.history.pushState(null, '', `?email=${encodeURIComponent(email)}&resent=true`);
  };

  return (
    <div 
      id="email-expired-page-container" 
    ...
    >
      ...
      <div className="w-full max-w-lg flex flex-col gap-6 relative z-10">
        ...
        {/* Header Logo */}
        <div id="expired-brand-logo" className="flex flex-col items-center text-center gap-2 select-none">
        ...
        >
          {/* Decorative Icon displaying timeout */}
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="text-xs sm:text-sm text-brand-muted leading-relaxed max-w-sm">
              Por razões de segurança, os links de ativação expiram após 24 horas. Solicite um novo link abaixo para confirmar a posse de sua conta corporativa.
            </p>
          </div>
          ...
          <div className="w-full px-4 py-3 bg-brand-canvas border border-brand-border rounded-2xl flex items-center justify-center gap-2 max-w-sm">
            <span className="text-xs text-brand-muted">Destinatário:</span>
            <strong className="text-xs text-brand-dark truncate">{email}</strong>
          </div>

          {resent && (
            <div 
              id="expired-success-feedback" 
            ...
            >
              ...
              <span>Um novo link de ativação válido foi enviado para o seu e-mail!</span>
            </div>
          )}
          ...
          >
            ...
            <span>Solicitar novo e-mail de ativação</span>
          </button>
